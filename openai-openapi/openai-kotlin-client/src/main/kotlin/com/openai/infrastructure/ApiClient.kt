...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        // This static field seems like such a bad idea! :/
    ...
    inline fun <reified T: Any?> responseBody(response: Response, mediaType: String? = JsonMediaType): T? {
        ...
        return when {
            ...
            mediaType.startsWith("text/") && mediaType.endsWith("plain") ->
                body.string() as T
            mediaType == OctetMediaType ->
                body.bytes() as? T
            else ->  throw UnsupportedOperationException("responseBody currently only supports JSON body and text/plain body.")
        }
    }
